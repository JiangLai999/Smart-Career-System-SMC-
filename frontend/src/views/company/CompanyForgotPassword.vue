<template>
  <div class="flex min-h-screen bg-background-light dark:bg-background-dark">
    ...
    <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-primary to-primary/80">
      <div class="absolute inset-0 z-0">
        <div class="w-full h-full bg-cover bg-center" 
          style="background-image: linear-gradient(rgba(19, 127, 236, 0.9), rgba(19, 127, 236, 0.7)), url('https://images.unsplash.com/photo-1497366216548-37526070297c?w=800')">
        </div>
      </div>
      <div class="relative z-10 flex flex-col justify-between p-12 text-white w-full">
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-2xl font-bold tracking-tight">企业招聘系统</h1>
        </div>
        <div class="max-w-md">
        ...
        <div class="flex items-center gap-4 text-sm text-white/70">
          <span>© 2024 企业招聘平台. All rights reserved.</span>
        </div>
      </div>
    </div>
      ...
      <div class="w-full max-w-[480px] bg-white dark:bg-slate-900 p-8 md:p-10 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <div v-if="step === 1">
          ...
          <form @submit.prevent="sendCode" class="space-y-5">
            <div class="space-y-2">
              <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">账号</label>
              <div class="relative">
              ...
              <p class="text-xs text-slate-500 dark:text-slate-400">支持手机号或邮箱</p>
            </div>
        ...
        <div v-if="step === 4" class="text-center py-8">
          ...
          <router-link to="/company/login" class="inline-block px-8 py-3 bg-primary text-white font-bold rounded-lg hover:bg-primary/90 transition-colors">
            立即登录
          </router-link>
        </div>
        ...
        <div v-if="step < 4" class="mt-8 text-center">
          <p class="text-slate-600 dark:text-slate-400">
            想起密码了？ <router-link to="/company/login" class="text-primary font-bold hover:underline">返回登录</router-link>
          </p>
        </div>
      </div>
    </div>
...
const sendCode = async () => {
  ...
  if (!phoneRegex.test(form.value.account) && !emailRegex.test(form.value.account)) {
    alert('请输入正确的手机号或邮箱')
    return
  }
...
const resetPassword = async () => {
  ...
  loading.value = true
  try {
    await api.companyResetPassword(form.value.account, form.value.code, form.value.newPassword)
    alert('密码重置成功')
    step.value = 4
  } catch (error) {
